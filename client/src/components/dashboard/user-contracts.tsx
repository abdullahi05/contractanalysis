...
export default function UserContracts() {
    ...
    const contractTypeColors: { [key: string]: string } = {
        ...
        "Lease": "bg-pink-100 text-pink-800 hover:bg-pink-200",
        "Services": "bg-orange-100 text-orange-800 hover:bg-orange-200",
        "Partnership": "bg-emerald-100 text-emerlad-800 hover:bg-emerald-200",
    ...
    const columns: ColumnDef<ContractAnalysis>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <Badge className="rounded-md" variant={
    ...
    const averageScore =
        totalContracts >
            0 ? (contracts?.reduce((sum, contract) => sum + (contract.overallScore ?? 0), 0) ?? 0) / totalContracts : 0


    const highRiskContracts =
        contracts?.filter((contracts) =>
            contracts.risks.some((risk) => risk.severity === "high")
        ).length ?? 0;
    return (
