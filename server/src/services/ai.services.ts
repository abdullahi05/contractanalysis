...
// first service is extract text from pdf
...
export const detectContractType = async (
    ...
    it is. Provide only the contract type as a single string (e.g., "Employment", "Non-Disclosure", "Sales", "Lease", etc)
...
export const analyzeContractWithAI = async (
  ...
  if (tier === "premium") {
    ...
    try {
        // attempt tp fix common js errors
        ...
        text = text.replace(/```(?:json)?/g, "");  // Remove ```json or ```
    ...
    const fallbackAnalysis: FallbackAnalysis = {
        risks: [],
        opportunities: [],
        summary: "Error analysis contract"
    };
